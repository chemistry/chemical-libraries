...
export class SpaceGroup {
  public static getById(id: number): SpaceGroupInfo | null {
  ...
  public static getByHMName(hmName: string): SpaceGroupInfo | null {
  ...
  public static getByHallName(hallName: string): SpaceGroupInfo | null {
    const hs = hallName || '';
  ...
  public static getCrystalSystem(sg: SpaceGroupInfo): CrystalSystem | null {
    if (sg.id >= 1 && sg.id <= 2) {
  ...
  public static getUnitCellCentring(sg: SpaceGroupInfo): UnitCellCentring | null {
  ...
  public static getCentringType(sg: SpaceGroupInfo): UnitCellCentringType | null {
    const ucc = SpaceGroup.getUnitCellCentring(sg);
    if (ucc === UnitCellCentring.P) {
      return UnitCellCentringType.Primitive;
    }
    if (ucc === UnitCellCentring.A || ucc === UnitCellCentring.B || ucc === UnitCellCentring.C) {
  ...
  public static getUniqueSpaceGroupsList(): { id: number; hm: string; hs: string }[] {
    const resList: { id: number; hm: string; hs: string }[] = [];
